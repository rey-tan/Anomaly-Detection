...
def test_register_request_sends_otp_and_verifies_email(test_db):
    ...
    with patch("src.api.app.send_otp_email") as mocked_send_email:
...
def test_register_request_duplicate_username_or_email_fails(test_db):
    ...
    with patch("src.api.app.send_otp_email"):
        response = client.post(
            "/register/request",
            json={
                "username": "duplicate",
...
def test_register_request_rejects_non_analyst_role(test_db):
    client = TestClient(app)
    ...
    assert "analyst role" in response.json()["detail"]

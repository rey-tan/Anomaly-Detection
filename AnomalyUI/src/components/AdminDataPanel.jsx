...
export default function AdminDataPanel({ token }) {
  ...
  const minDate = adminState.previewData?.last_date ? fmtDate(adminState.previewData.last_date) : "";
  ...
  useEffect(() => {
    if (adminState.previewData) {
      const s = fmtDate(adminState.previewData.last_date) || "";
      const e = todayDate;
      setScrapePayload((p) => ({
        ...p,
        ...
        <div style={{ display: "grid", gap: 12 }}>
            ...
            {adminSymbols.length ? (
              ...
              return filtered.map((name) => (
                <div key={name} className="admin-user-card">
                  <div className="data-content">
                      ...
                      <div style={{ fontWeight: 700 }}>{name}</div>
                        ...
                        await loadPreview(name);
                                ...
                                onClick={async () => {
                                    try {
                        await downloadAdminFile(token, `${name}.csv`);
                                    } catch (err) {
    ...
    <div className="page-panel" style={{ height: "max-content" }}>
        <div className="admin-panel">
            ...
            <form className="admin-form" onSubmit={handleRunScrape}>
                ...
                <select className="admin-select" value={scrapePayload.source} onChange={(e) => setScrapePayload((p) => ({ ...p, source: e.target.value }))} disabled={scraping}>
                  <option value="sharesansar">sharesansar</option>
                </select>
                <input type="date" className="admin-input" value={scrapePayload.start_date} min={minDate} max={maxDate} onChange={(e) => setScrapePayload((p) => ({ ...p, start_date: e.target.value }))} disabled={scraping} />
                <input type="date" className="admin-input" value={scrapePayload.end_date} min={minDate} max={maxDate} onChange={(e) => setScrapePayload((p) => ({ ...p, end_date: e.target.value }))} disabled={scraping} />
                <select className="admin-select" value={scrapePayload.output_format} onChange={(e) => setScrapePayload((p) => ({ ...p, output_format: e.target.value }))} disabled={scraping}>

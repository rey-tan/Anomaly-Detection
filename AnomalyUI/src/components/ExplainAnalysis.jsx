...
import {  extractMetricsAndParams, isAnomalyRow } from '../utils/analysisHelpers';
...
export default function ExplainAnalysis({token,results,selectedAnalysis,flaggedCount}) {
    ...
    const anomalyRows = useMemo(
        () =>
            (results?.data || []).filter(isAnomalyRow),
        [results]
    );
            ...
            const contextualRows = anomalyRows.map((row) => {
                ...
                return {
                    ...
                    detector_flags: [
                        ...
                        row.dbscan_label === -1 || row.cluster_dbscan === -1 ? "DBSCAN" : null,
                        row.isolation_forest_label === -1 || row.cluster_isolation_forest === -1 ? "Isolation Forest" : null,
                    ].filter(Boolean),
